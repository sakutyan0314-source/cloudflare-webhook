...
- Market Analysis v1は`market-signal-analysis-input-v1`をOpenAI Responses APIの`gpt-5.6-terra`へ最大1 callだけ渡し、strict JSONの`market-signal-analysis-v1`を検証するplanning-only層である。入力は最大10件のSERP metadata、既存記事metadata overlap、Search Console summary、affiliate summary、既存intent taxonomyだけであり、競合本文・記事本文・raw response・Secretは含めない。retry/fallback/toolsなし、`store=false`、input 1,800/output 600 tokens上限、candidate draft最大3件、全draftのhuman review必須・権限falseを固定する。`uncovered_questions`は`possible_gap`または`hypothesis`だけで、`already_covered`・`high_duplicate_risk`の新規candidate化は拒否する。live pathはcache envelope/version・normalized result・analysis input・Responses message/output_text・local validatorを各境界でfail-closed検証する。`--analysis-preflight`は同じquery、期間、production D1 fixed SELECT、TTL内のnormalized SERP cache、own-site signal、analysis input builder/validator、OpenAI payload builderを通し、送信だけを省くcache-only zero-call modeである。成功時は本文を出さないcanonical input fingerprintとcomponent hashだけを返し、cache miss・D1 read・型/field validation失敗も安全なrule/stage metadataだけで停止する。`--preflight-to-live-identity-guard`は同一processでcache-only preflight inputを一度だけ構築し、そのcanonical fingerprintをOpenAI送信直前の同一input objectと照合する。mismatch時はOpenAI call 0で停止し、hard-codedの過去fingerprintには依存しない。transportからCLIまではHTTP status、timeout、credential/configuration、connection reset/close、response read/decode、unknown delivery、incomplete/refusal/output type/content type/JSON/provider HTTPを本文非表示の固定classificationとdelivery stateだけで伝播する。参考価格（判定ロジックには不使用）はshort-context input $2.00 / output $12.00 per 1M tokensで、上限時のLLM理論目安は約$0.0108/analysisである。
