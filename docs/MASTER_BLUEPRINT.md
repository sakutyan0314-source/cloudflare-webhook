...
- Market Analysis v1は`market-signal-analysis-input-v1`をOpenAI Responses APIの`gpt-5.6-terra`へ最大1 callだけ渡し、strict JSONの`market-signal-analysis-v1`を検証するplanning-only層である。入力は最大10件のSERP metadata、既存記事metadata overlap、Search Console summary、affiliate summary、既存intent taxonomyだけであり、競合本文・記事本文・raw response・Secretは含めない。retry/fallback/toolsなし、`store=false`、input 1,800/output 600 tokens上限、candidate draft最大3件、全draftのhuman review必須・権限falseを固定する。`uncovered_questions`は`possible_gap`または`hypothesis`だけで、`already_covered`・`high_duplicate_risk`の新規candidate化は拒否する。参考価格（判定ロジックには不使用）はshort-context input $2.00 / output $12.00 per 1M tokensで、上限時のLLM理論目安は約$0.0108/analysisである。
