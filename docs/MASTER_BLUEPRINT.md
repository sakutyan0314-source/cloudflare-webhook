...
- `0011_approved_content_authorization_bundles.sql` は、既存Topic Candidate / human review / approved planning / content handoff / approved production input / Content Production Approvalの**content-free immutable snapshot**を1 tableへ保存するmigrationであり、本番適用済みである。production inputとapprovalはUNIQUEであり、本文・AI出力・Secret・tokenは保存しない。適用後にmigration record、table、index/UNIQUE制約、FK checkを確認し、bundle 0件、`production_executions` 0件、既存article/pipeline run件数不変を確認した。Workerの`/internal/approved-canary`はbundleを固定SELECTで復元し、snapshot fingerprint、identity、review decision、expiry、single-use、および`production_executions`未使用をGemini前に検証する。bundleなし・期限切れ・改ざん・既存実行はfail-closedとし、Cronはこの経路を起動しない。
...
- Topic-aware production input adapterは、既存のTopic Candidate/review/production input/approval validator群で生成したcontent-free briefだけを既存Gemini instruction builderへ渡す。`runReliablePipeline()`はGemini開始前にbriefをfail-closed検証し、Claude、OpenAI、既存SEO quality gate、保存・通知の順序と通常Cron/manualの挙動は変えない。single-useは既存approved-canaryとpipeline idempotency境界を再利用し、新たなapproval・publication境界は追加しない。
- **FIRST TOPIC-AWARE PRODUCTION CANARY: CLOSED / SUCCESS.** `production_execution_83b5f4407a92504e3db278cc` は`outcome_known_success / success`で完了した。bundle `bundle_ef09fa777c15f5488c391956`、`production_input_3abb8107da393a9f36498db6`、`production_approval_f9a2010f4da58750942ed9b5`のidentityを検証し、pipeline run `18`は`completed / done`、記事`40`（`【2026年最新】Microsoft 365 Copilot エージェント導入ガバナンスの全貌：組織の未来を守るための5ステップ`、`security-governance`）を1件保存した。`/article/40`はHTTP 200・canonical PASS、SEO quality gateはPASS（audit `7289532e-c8f8-4b7b-870e-df1957a6df2f`）、Discordは1回でsent。single-useはconsumed、duplicate execution/idempotency duplicateはなく、Gemini → Claude → OpenAI → quality gate → article save → notification、およびapproved-canary専用各1回retry policyが本番で完走した。
...
- Market Signal Report v1は外部SERP API接続前のread-only planning層である。競合本文・raw provider response・Secretは保存／AI入力に含めず、normalized SERP metadata（最大10件）と既存記事metadata、Search Console、affiliate eventを結合して最大3件のnon-executable candidate draftを出す。draftは既存Topic Candidate validatorとhuman reviewの前段であり、approval、生成、publication権限を持たない。affiliateのDiscord placementは通知直後の自動アクセスと区別できず、human clickとして需要評価へ加算しない。
- SerpApi Google adapterは`SERPAPI_API_KEY`環境変数だけを使用し、明示`--live-serp`時だけ1 normalized queryにつき最大1 request、retryなしで呼ぶ。日本向けGoogle organic上位10 metadataを既存normalizerへ渡し、local gitignored cacheにはnormalized metadataだけを設定可能TTLで保存する。key欠落、HTTP/provider/schema異常、2回目requestはcandidate draft前にfail-closedする。通常fixture CLI、D1、Worker、pipeline、Topic Candidate approvalには接続しない。
- Market Analysis v1は`market-signal-analysis-input-v1`をOpenAI Responses APIの`gpt-5.6-terra`へ最大1 callだけ渡し、strict JSONの`market-signal-analysis-v1`を検証するplanning-only層である。入力は最大10件のSERP metadata、既存記事metadata overlap、Search Console summary、affiliate summary、既存intent taxonomyだけであり、競合本文・記事本文・raw response・Secretは含めない。retry/fallback/toolsなし、`store=false`、input 1,800/provider output budget 2,400 tokens、candidate draft最大3件、全draftのhuman review必須・権限falseを固定する。2,400はreasoningとvisible structured outputを書き切るprovider budgetであり、最終JSONの簡潔さはlocal validatorのfield/prose/policy制約で維持する。strict schema互換上表現できない候補数・field length・metadata-only prose・unique intent制約は、モデルinstructionとlocal validatorの両方で明示する。candidate schemaは`cluster_sibling`/`possible_gap`および`none`/`low`/`medium` riskだけを許可し、`already_covered`・`high_duplicate_risk`の新規candidate化は拒否する。schema/policy拒否時は本文を出さず、`validation_rule`・field/type/count/length・`policy_code`だけをCLI JSONへ伝播する。live analysis成功時だけは、allowlist化した`market-signal-local-report-v1`をgitignore済み`.market-signal-reports/`へatomic local保存し、terminalには短いhuman-readable summaryとpathを表示する。保存対象はanalysis／candidate drafts／own-site summary／usage／安全flagだけであり、SERP URL/snippet/results、raw provider response、本文、Secret、error logは保存しない。local reportはapproval・production・publication権限を持たず、human review前の参照専用である。live pathはcache envelope/version・normalized result・analysis input・Responses message/output_text・local validatorを各境界でfail-closed検証する。own-site production readは独自subprocess実装を持たず、Phase 2Aで実証済みの`WranglerFixedSelectTransport`を直接再利用する。`--analysis-preflight`は同じquery、期間、production D1 fixed SELECT、TTL内のnormalized SERP cache、own-site signal、analysis input builder/validator、OpenAI payload builderを通し、送信だけを省くcache-only zero-call modeである。成功時は本文を出さないcanonical input fingerprintとcomponent hashだけを返し、cache miss・D1 read・型/field validation失敗も安全なrule/stage metadataだけで停止する。`--preflight-to-live-identity-guard`は同一processでcache-only preflight inputを一度だけ構築し、そのcanonical fingerprintをOpenAI送信直前の同一input objectと照合する。mismatch時はOpenAI call 0で停止し、hard-codedの過去fingerprintには依存しない。transportからCLIまではHTTP status、timeout、credential/configuration、connection reset/close、response read/decode、unknown delivery、incomplete/refusal/output type/content type/JSON/provider HTTPを本文非表示の固定classificationとdelivery stateだけで伝播し、usageのinput/output/reasoning token数だけを安全に出せる。参考価格（判定ロジックには不使用）はshort-context input $2.00 / output $12.00 per 1M tokensで、2,400 token provider budget時のLLM理論上限は約$0.0324/analysisである。
- **MARKET SIGNAL / MARKET ANALYSIS V1 — CLOSED / SUCCESS（2026-08-27）**: SerpApi live Google SERP取得、normalized SERP 9件、cache、Market Signal Report、共通`WranglerFixedSelectTransport`によるown-site D1 fixed SELECT、Search Console／affiliate signal統合、OpenAI `gpt-5.6-terra` single-call analysis（retry 0・fallbackなし）、preflight-to-live identity guard、strict structured output＋local validator、sanitized local report persistenceを実環境で確認した。live queryは「Microsoft 365 Copilot エージェント」。観測intentは`what`、`how`、`business_use`、`compare`、`commercial_investigation`であり、定義／通常Copilotとの差分、業務別活用、自動化、作成方法・方式、組織導入・共有・管理・security、料金・利用条件、他AIとの比較を主要angleとして抽出した。候補例は「導入前チェックリスト」「業務別の作成／非作成の選定基準」「宣言型・自律型・Copilot Studioの作成方式比較」。candidate draftは最大3件、human review必須、content generation／execution／publication権限はすべてfalseで停止する。Search Consoleは`insufficient_data`、affiliateはarticle click 1件を弱いusable signal、Discord clickはhuman status unknownのため不採用とする。実行usageはSerpApi 0 request（cache）、OpenAI 1 call、input/output/reasoning tokens = 2081/1286/0。raw SerpApi／OpenAI response、競合本文、Secret/API keyは保存せず、sanitized reportだけを`.market-signal-reports/`へlocal保存する。D1 write 0、Worker changeなし、migrationなし、article generation 0、publication 0。
- **PROJECT STATUS — PAUSED AT CLEAN CHECKPOINT**: productionは正常、Market Signal / Market Analysis V1はclosed/success、次工程・migration・deploy・production executionに未着手の作業はない。次回の候補は「Market Signal / Market Analysis V1の定期運用化と自己改善ループへの接続」であり、`Market Signal → candidate draft → human review → Topic Candidate → article generation → Search Console / affiliate / revenue / cost → 成果評価 → 次回Market Signal`の順に検討する。これは再開時の候補であり、本クローズでは実装しない。
