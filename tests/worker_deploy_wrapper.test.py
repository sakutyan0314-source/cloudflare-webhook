...
class T(unittest.TestCase):
 def execute(self,code=0,out='Total Upload:\nDeployed',err='',exc=None):
  ...
  def f(args,cwd):
   ...
   if exc: raise exc
   return code,out,err
  x=w.run_deploy(root=ROOT,git_head='h',account=w.ACCOUNT,runner=f);return x,calls
 ...
 def test_preflight(self):
  x=w.run_deploy(root=ROOT,git_head='h',account='wrong',runner=lambda *_:(_ for _ in ()).throw(Exception()));self.assertEqual('preflight_failed',x.classification)
