...
const canary = await fs.readFile(new URL("src/approved_canary_worker_adapter.js", root), "utf8");
...
source = source.replaceAll('"./approved_canary_worker_adapter.js"', `"data:text/javascript;base64,${Buffer.from(canary).toString("base64")}"`)
  .replaceAll('"./publication_worker_adapter.js"', `"data:text/javascript;base64,${Buffer.from(publication).toString("base64")}"`);
...
const env = {
  ...
  APPROVED_CANARY_RUNTIME: { resolve: async (payload) => ({ request: { trigger_type: "approved_canary", production_input: {}, approval: {}, production_execution_id: payload.production_execution_id, pipeline_run_id: 1, brief: { production_input_id: payload.production_input_id, topic_candidate_id: "topic", human_review_id: "review", topic: "x", title_hint: "y", primary_intent: "how", target_audience: "z", problem_to_solve: "p", cluster_id: "c", internal_link_guidance: {}, ai_generation_authorized: false, publication_authorized: false, execution_authorized: false } }, validate: async () => calls.push("validate"), executionRepository: { acquire: async () => "e", transition: async (_e, s) => s, linkPipelineRun: async () => {}, linkQualityGateAudit: async () => {} }, pipeline: async () => ({}), qualityGate: async () => ({ classification: "pass", audit_id: "audit" }), staging: { create: async () => calls.push("staging") } }) },
...
await test("authenticated valid POST reaches only its adapter", async () => { calls=[]; const r=await request("/internal/approved-canary", { method:"POST", headers, body:JSON.stringify(production) }); assert.equal(r.status,202); assert.deepEqual(calls,["validate","staging"]); });
