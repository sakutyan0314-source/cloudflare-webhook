...
const production = { trigger_type: "approved_canary", production_input_id: "input", approval_id: "approval", production_execution_id: "execution" };
...
await test("production entry rejects caller-supplied pipeline run identity", async () => { calls=[]; for (const payload of [{...production,pipeline_run_id:1},{...production,pipeline_run_id:"1"}]) { const r=await request("/internal/approved-canary",{method:"POST",headers,body:JSON.stringify(payload)}); assert.notEqual(r.status,202); } assert.deepEqual(calls,[]); });
await test("canary route records known pipeline failures separately from unknown transport outcomes", async () => { assert.match(source, /error\?\.deliveryUnknown === true\) await recordApprovedCanaryOutcomeUnknown/); assert.match(source, /else await finalizeApprovedCanary\(env\.DB, resolved, \{ result: null/); });
