...
await test("valid immutable bundle resolves to topic-aware brief",async()=>{const item=await row();const out=await runtime.resolveApprovedCanaryBundle(dbFor(item),payload,new Date("2026-08-22T19:00:00.000Z"));assert.equal(out.brief.topic,"topic");assert.equal(out.specification.idempotencyKey,"manual:topic:input_one");assert.deepEqual(out.specification.approvedCanaryMaxAttempts,{gemini:1,claude:1,openai:1,discord:1});});
